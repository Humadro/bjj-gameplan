...
- **Command palette:** `CommandPalette` (editor only) opens on **⌘/Ctrl-K**. Free-text: `desde X > nombre > Y, alta` → technique (positions matched case-insensitively or created via the `__new__` path), `pos nombre [mala]` → position, `ir nombre` → jump to another map. Enter with no `>` / an unparseable line drops to a small quick-form (name + `desde`/`lleva a` with a `<datalist>` of position names + confidence). Submits through the existing `createTechnique` / `createPosition` actions; stays open for the next entry.
